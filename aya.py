...
def run(MODEL_NAME):
    ...
    print("...example...")
    generations = generate_aya_23(prompts, model, tokenizer, prompt_type=1)
    ...
    try:
        generations_system = generate_aya_23(prompts, model, tokenizer, prompt_type=2)
        for p, g in zip(prompts, generations_system):
            print(
                "PROMPT", p ,"RESPONSE", g, "\n", sep="\n"
                )
    except Exception as e:
        print("cannot work with system template")
        print(e)

    print("...few shot example...")
    try:
        generations_few_shot = generate_aya_23(prompts, model, tokenizer, prompt_type=3)
        for p, g in zip(prompts, generations_few_shot):
            print(
                "PROMPT", p ,"RESPONSE", g, "\n", sep="\n"
                )
    except Exception as e:
        print("cannot add examples")
        print(e)


if __name__ == "__main__":

    for model_name in ["bigscience/bloomz-7b1", "mistralai/Mistral-7B-Instruct-v0.3"] #"meta-llama/Meta-Llama-3-8B-Instruct", "CohereForAI/aya-23-8B"]:
        print(model_name)
        run(model_name)
        cleanup()
        print()