...
for lang in $languages; do
    ...
    comet-score -s ${src_file} -t ${hyp_file} -r ${ref_file} --model Unbabel/wmt22-comet-da --gpus 1 --only_system
    #xx2en
    hyp_file="${prefix_out}/$system.${lang}2en.txt"
    comet-score -s ${ref_file} -t ${hyp_file} -r ${src_file} --model Unbabel/wmt22-comet-da --gpus 1 --only_system
done